...
from fastapi.staticfiles import StaticFiles              # ← NEW (optional, for static/)
...
# If you ever add CSS/JS files, put them in a static/ folder and uncomment:
...
async def tmdb_get(path: str, params: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # We add verify=False to ignore SSL errors and follow_redirects for speed
    try:
        async with httpx.AsyncClient(
            ...
            if r.status_code != 200:
                print(f"TMDB ERROR: Status {r.status_code}, Body: {r.text}")
                raise HTTPException(status_code=502, detail=f"TMDB rejected request: {r.status_code}")
                
            return r.json()
    except httpx.ConnectTimeout:
        raise HTTPException(status_code=504, detail="TMDB Connection Timed Out. Check your network.")
...
def load_pickles():
    ...
    print(f"DEBUG: Loaded API Key is: {TMDB_API_KEY}")
...
# =========================
# ★  SERVE THE FRONTEND  ★   ← NEW
# =========================
@app.get("/", include_in_schema=False)
async def serve_frontend(request: Request):
    """Serves templates/index.html at http://localhost:8000/"""
...
async def home(
    ...
    try:
        ...
        data = await tmdb_get(endpoint, {"language": "en-US", "page": 1})
